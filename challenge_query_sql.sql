...
-- VIEW TABLE
--SELECT *FROM retirees_by_dept_summary
...
/*SELECT emp_no, 
	...
	from_date,
	salary
INTO unique_retirees_by_dept
FROM 
(SELECT emp_no, first_name, last_name, title, from_date, salary, ROW_NUMBER() OVER 
(PARTITION BY (emp_no)
ORDER BY from_date DESC) rn
FROM retirees_by_dept_summary
) tmp WHERE rn = 1
ORDER BY title;
*/

--VIEW TABLE
SELECT *FROM unique_retirees_by_dept